...
export const ProfileView = () => {
  ...
  const {
    ...
    retry: false, // don't retry on error so we can catch a 404 immediately
  });
  ...
  // If an error occurs and it is NOT a 404, show an error message.
  if (error && !(error.response && error.response.status === 404)) {
    return <div>Error loading profile.</div>;
  }

  // If there is no profile (either error 404 or profile is null), show the edit form.
  if (!profile) {
    return (
  ...
  // If profile exists, show the profile details and option to edit.
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
      <Toaster />
      {!isEditing ? (
